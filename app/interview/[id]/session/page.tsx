...
export default function InterviewSessionPage() {
  ...
  const isProcessingRef = useRef(false);
  const [typedAnswer, setTypedAnswer] = useState('');
  // FIX 2: Use a ref for typedAnswer to avoid stale closure in completeRef
  ...
  // FIX 1: Update completeRef on every render so it never holds a stale closure
  useEffect(() => {
    completeRef.current = handleCompleteInterview;
  });
  ...
  const handleCompleteInterview = async () => {
    ...
    // FIX 2: Use typedAnswerRef instead of typedAnswer state to avoid stale closure
    if (typedAnswerRef.current.trim()) {
      fullConversation.push({ role: 'user', content: typedAnswerRef.current.trim() });
    }

    console.log(`🤖 Evaluating ${fullConversation.length} messages with Claude...`);

    try {
      // FIX 3: Wrap in Promise.race with 15s timeout so redirect always fires
      await Promise.race([
        evaluateAndComplete(
          interviewId,
          fullConversation,
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Evaluation timeout after 15s')), 15000)
        ),
      ]);
    } catch (e) {
      console.error('evaluateAndComplete failed (still redirecting):', e);
    } finally {
      // FIX 4: Always reset processing state so it can't get permanently stuck
  ...
  // FIX 1: completeRef is now updated via useEffect above — removed from render body
  ...
  const handleStartCall = async () => {
    try {
      ...
      setSubtitles('');
      setTypedAnswer('');
      typedAnswerRef.current = ''; // FIX 2: sync ref
      setAnswerSubmitted(false);
      setConversation([]);
      conversationRef.current = [];
      await startInterviewCall({
  ...
  const handleSubmitTypedAnswer = () => {
    ...
    setAnswerSubmitted(true);
    setTypedAnswer('');
    typedAnswerRef.current = ''; // FIX 2: sync ref on clear
  };

  // FIX 2: Keep typedAnswerRef in sync whenever typedAnswer changes
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    value={typedAnswer}
                    onChange={(e) => handleTypedAnswerChange(e.target.value)} // FIX 2: use synced handler
                    onKeyDown={(e) => {
                      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
                        e.preventDefault();
                        handleSubmitTypedAnswer();
